...
class PedidoController
{
    ...
    public function getPedido($cliente, $data){
        $pedido = null;
    ...
    public function inserirItensPedido($pedido, $produto, $quantidade, $valorProduto){
        $sql = "INSERT INTO itempedido (pedido_id, produto_id, quantidade, valor_produto ) VALUES (:pedido, :produto, :quantidade, :valorproduto )";
        $statement = $this->conexao->prepare($sql);
        $statement->bindValue(":pedido", $pedido);
        $statement->bindValue(":produto", $produto);
        $statement->bindValue(":quantidade", $quantidade);
        $statement->bindValue(":valorproduto", $valorProduto);
        return $statement->execute();
    }
    }
